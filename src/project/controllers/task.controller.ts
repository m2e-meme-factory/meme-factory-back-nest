...
export class TaskController {
  ...
  @ApiOperation({
    ...
    description: 'Позволяет пользователю-исполнителю подать заявку на завершение задачи в рамках проекта.',
  })
  @ApiParam({
    name: 'taskId',
  ...
  @ApiBody({
    description: 'Сообщение от исполнителя при подаче заявки на завершение задачи.',
    schema: {
      type: 'object',
      properties: {
        message: {
  ...
  @ApiBody({
    description: 'Идентификатор исполнителя и сообщение при одобрении завершения задачи.',
    schema: {
      type: 'object',
      properties: {
        creatorId: {
  ...
  @ApiResponse({ status: 404, description: 'Задача или прогресс проекта не найдены.' })
  @ApiResponse({ status: 500, description: 'Ошибка при одобрении завершения задачи.' })
