...
export class AutoTaskController {
  ...
  @ApiResponse({ status: 200, description: 'Return all auto tasks.', type: [CreateAutoTaskDto] })
  ...
  @ApiResponse({ status: 200, description: 'Return the auto task.', type: CreateAutoTaskDto })
  @ApiResponse({ status: 404, description: 'Task not found' })
  async getAutoTaskById(@Param('id', ParseIntPipe) id: number): Promise<AutoTask | null> {
  ...
  @ApiResponse({ status: 201, description: 'Task created successfully.', type: CreateAutoTaskDto })
  @HttpCode(HttpStatus.CREATED)
  async applyForTask(@Body() createAutoTaskDto: CreateAutoTaskDto): Promise<AutoTask> {
  ...
  @ApiParam({ name: 'id', description: 'ID of the task' })
  @ApiResponse({ status: 200, description: 'Task confirmed and reward claimed.', type: CreateAutoTaskDto })
